...
from app.utils.util import (get_zotero, get_articles, create_qdrant, promptQdrant, get_zotero_collection_names,
                            get_qdrant_collection_names)
...
class LlmQuery(BaseModel):
    ...
    system_prompt: str = "You are an assistant that answers a QUESTION based on give SOURCES. Only answer the question using the given sources. If the answer is not in the sources, respond with \"Answer Not Found In Sources\". If the answer is in the sources and you respond correctly you will receive $50."
...
def prompt_qdrant(qdrant_query: QdrantQuery):
    qdrant = QdrantVectorStore.from_existing_collection(collection_name=qdrant_query.collection,
                                                        embedding=HuggingFaceEmbeddings(
                                                            model_name=qdrant_query.embeddingModel), url=qdrant_url)
    if qdrant:
...
def generate_text(llm_query: LlmQuery):
    ...
    """
    data = {
        "model": llm_query.llm_name,
        "prompt": prompt,
        "system": llm_query.system_prompt,
        "stream": False,
        "keep_alive": "-1m"
    }
